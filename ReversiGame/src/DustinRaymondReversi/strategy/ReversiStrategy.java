...
/**
 * Represents a strategy used to select cells to play at in a Reversi game. The
 * interface contains a single method that returns a list of HexPosns (in axial
 * coordinates) representing the best cells in the Reversi game (provided as a
 * read-only Reversi model) at which to place a piece for the current player in 
 * the game, based on some kind of criteria for deciding which moves would be best
