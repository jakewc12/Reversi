 ...
 * numerous) moves that might be returned by a particular Reversi strategy. If the strategy
 * fails to return any moves, the tiebreaker returns an empty value; otherwise, it picks one
 * of the values returned by the strategy and returns that one according to some particular
 * rule.
 */
public interface StrategyTiebreaker {
  /**
   * Picks one particular move among all the possible moves that are returned by the given
   * strategy. If the strategy fails to find any moves, returns an empty.
   *
